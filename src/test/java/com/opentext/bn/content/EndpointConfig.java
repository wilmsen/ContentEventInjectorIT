...
public class EndpointConfig implements WebMvcConfigurer {
	...
	@Bean
	public Map<String, Object> xifSenderProperties() {
	...
	@Bean
	public Map<String, Object> xifListenerProperties() {
		Map<String, Object> props = new HashMap<>();
		props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
		props.put(ConsumerConfig.GROUP_ID_CONFIG, "CitrusTest");
	...
	public KafkaEndpoint legacyKafkaEndpoint() {
		...
		return CitrusEndpoints.kafka().asynchronous().server("localhost:9092")
				.topic("visibility.platform.introspection").keyDeserializer(StringDeserializer.class)
	...
	public KafkaEndpoint citrusKafkaEndpoint() {
		return CitrusEndpoints.kafka().asynchronous().producerProperties(xifSenderProperties()).build();
	}

	/**
	 * "", "visibility.platform.taskcompleted", "visibility.platform.taskerror",
	 * "visibility.platform.receivecompleted",
	 * "visibility.platform.receiveerror",
	 * "visibility.platform.deliverycompleted",
	 * "visibility.platform.deliveryreadyforpickup",
	 * "visibility.platform.deliveryerror", "visibility.platform.document",
	 * "visibility.platform.envelope", "visibility.platform.introspection",
	 * "visibility.platform.contenterror", "visibility.introspection.document",
	 * "visibility.introspection.envelope",
	 * "visibility.introspection.contenterror", "visibility.fgfa.status",
	 * "visibility.notificationrequest", "visibility.internal.facycle",
	 * "visibility.internal.fgfastatus"
	 * 
	 */
